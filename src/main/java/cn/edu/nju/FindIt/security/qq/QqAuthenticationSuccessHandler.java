 ...
 * 用户认证通过的处理handler
 */
public class QqAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {
        ...
        String token = jwtTokenUtils.generateToken(((QqAuthenticationToken)authentication).getNJUID());
        Map<String, Object> result = Maps.newHashMap();
        result.put(ConstantEnum.AUTHORIZATION.getValue(), token);
        result.put("id", ((QqAuthenticationToken)authentication).getId().toString());
